...
public class Seven extends Agent {
	...
	//Added by Daniel
	/*
	 * The amount of campaigns we won
	 */
	private int campaignsWon;

	
	/*
	 * The active campaigns (out of the ones our agent won)
	 */
	...
	private void handleICampaignOpportunityMessage(
		...
		if(campaignsWon < 3 && pendingCampaign.campaignComplexity <= 3000){
			/* We haven't won 3 campaigns yet, so give an atractive price
			*  and the campaign isn't too hard for completion
			*/
			cmpBidMillis = ((int)cmpimps)/10;
		}
		else{
			/*
			* Either (1) won enough campaigns, so don't want to
			* win an un-randomly allocated campaign again for the 
			* duration of the current game OR (2)the campaign is
			* too difficult for commplition in the given time 
			*/
			//cmpBidMillis = ((int)cmpimps) + 1;
			cmpBidMillis = 1;
		}
	 ...
	 * auctions (for which the completing agents sent bids during day n -1) are
	...
	private void handleAdNetworkDailyNotification(
		...
		if ((pendingCampaign.id == adNetworkDailyNotification.getCampaignId())
			...
			//Added by Daniel
			//increase the counter of campaigns we won
			campaignsWon++;
			myActiveCampaigns.put(pendingCampaign.id, pendingCampaign);
	...
	protected void sendBidAndAds() {
		...
		noActiveCampaigns(dayBiddingFor);
	...
	private class CampaignData {
		...
		public CampaignData(InitialCampaignMessage icm) {
			...
			//Added by Daniel
			campaignComplexity = ((double)reachImps)/(((double)dayEnd)-((double)dayStart)+1.0);
		}
	...
	//Added by Daniel
	/*
	 * This function removes the unactive campaigns from the
	 * myActiveCampaigns map
	 */
	private void noActiveCampaigns(int dayBiddingFor){
		if(null != myActiveCampaigns && !(myActiveCampaigns.isEmpty())){
			for(CampaignData campaign: myActiveCampaigns.values()){
